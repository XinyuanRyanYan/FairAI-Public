 ...
 *      - get visual components but give them to other class
 */
class RenderChapterHandler{
    ...
    async reset(chapterId){
        ...
        this.VCLst = [];
        // first clean the text in this chapter
        ...
        // remove all newly added Visual Component
        d3.selectAll('.newVisualComponent').remove();
    ...
    // add fig
    renderFig(figJson){
        let figPath = '/static//uploads/img/'+figJson['name'];
        let caption = figJson['caption'];
        this.contentDiv.append('img').attr('src', figPath).attr('alt', 'fig').style('width', "500px").classed('imgContainer', 'true');
