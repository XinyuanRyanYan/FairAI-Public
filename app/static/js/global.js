...
/**
 * convert vw to px
 * used for safair browser
 * @param {*} vw 
 */
function vwTopxSf(vw){
    vw = parseFloat(vw);
    return `${document.documentElement.clientWidth/100*vw}px`;
}
 ...
 */
 function visTrainOrTestData(divSelector, data, title, weights=undefined, inputgroupsText=''){
    if(inputgroupsText==''){inputgroupsText = groupsText;}
    // clear
    ...
    divSelector.style('height', vwTopxSf(height));

    // add svg
    // const svg = divSelector.append('svg').style('width', `${ttdivDim.width}vw`).style('height', `${height}vw`);
    const svg = divSelector.append('svg')
        .style('width', vwTopxSf(ttdivDim.width))
        .style('height', vwTopxSf(height));

    // visualize bars
    // let chartsG = svg.append('g').style('transform', `translate(${ttdivDim.padding.left}vw, ${ttdivDim.padding.top}vw)`);
    let chartsG = svg.append('g').style('transform', `translate(${vwTopxSf(ttdivDim.padding.left)}, ${vwTopxSf(ttdivDim.padding.top)})`);
        ...
        .each(function(d, i){
            ...
            const visbar = (num, x, label, weight)=>{
                ...
                d3.select(this).append('rect')
                    // .attr('x', `${x}vw`).attr('y', `${scaleY(num)}vw`)
                    .attr('x', vwTopxSf(x)).attr('y', vwTopxSf(scaleY(num)))
                    // .attr('width', `${bandWidth}vw`)
                    .attr('width', vwTopxSf(bandWidth))
                    // .attr('height', `${innerHeight-scaleY(num)}vw`)
                    .attr('height', vwTopxSf(innerHeight-scaleY(num)))
                    .attr('fill', label==1? colorMap.orange : colorMap.blue)
                    .attr('border', 'none');
                if(hasWeight){
                    let addText = (pNum, anchor)=>{
                        return d3.select(this).append('text')
                                // .attr('x', `${x+bandWidth/2}vw`).attr('y', `${scaleY(num)}vw`)
                                .attr('x', vwTopxSf(x+bandWidth/2)).attr('y', vwTopxSf(scaleY(num)))
            ...
            d3.select(this).append('text').attr('x', vwTopxSf(centerX+bandWidth)).attr('y', vwTopxSf(innerHeight))
                .attr('text-anchor', 'middle')
                .attr('class', 'axisTick')
                .attr('dy', '1.2em')
                .text(inputgroupsText[i])
                .attr('class', 'ticks');
        });
    ...
    chartsG.append('line') 
        .attr('x1', 0)
        .attr('y1', vwTopxSf(innerHeight))
        .attr('x2', vwTopxSf(innerWidth))
        .attr('y2', vwTopxSf(innerHeight))
...
*/
function visOutputPanel(divSelector, data, colorMapDiv='', CFScale='', opacityScale='', accChange='', malePredChange='', femalePredChange='', inputgroupsText=''){
    ...
    visConfusionMatrixPanel(previlageCF, CFData[0], 0, false, opacityScale, '', malePredChange, inputgroupsText);
    visConfusionMatrixPanel(unprevilageCF, CFData[1], 1, false, opacityScale, '', femalePredChange, inputgroupsText);
 ...
 */
 function visConfusionMatrixPanel(divSelector, CFData, idx, interactive = false, opacityScale='', FairMetricsPanelObj, predChange='', inputgroupsText=''){
    if(inputgroupsText==''){inputgroupsText = groupsText;}
