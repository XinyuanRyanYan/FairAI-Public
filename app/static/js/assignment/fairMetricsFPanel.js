/**
 * A table shows the fairness metrics/Accuracy for the final project
 */
class FairMetricsFPanel{
    constructor(type, setPosition, VCSet) {
        ...
        this.type = type;  // type: metrics/accuracy
        ...
        this.metricNames = type!='acc'? ['SPD', 'DI', 'EOD', 'AOD']:['acc'];
        this.name = type!='acc'? 'FairMetricsF':'FairAccF';
    }
    ...
    async activate(){
        ...
        this.containerSelector.style('margin', '30px auto 30px auto;').style('overflow', 'scroll');
        this.containerSelector.style('display', 'block').style('height', '150px').classed("placeHolder", false);

        // 2. fetch the required data: metrics or accuracy
        if(this.type == 'acc'){
            await axios.post('/getAccuracy',{
                type: ''
            })
            .then((response)=>{
                console.log('response from the final project', response.data);
                let data = response.data;
                // 3. update the dim and resize the grid div
                this.dim.metricNum = this.metricNames.length;
                ...
                this.gridSelector.style('grid-template-columns', 
                    `${this.dim.dataWid}px ${this.dim.modelWid}px repeat(${this.dim.metricNum}, ${this.dim.metricsWid}px)`)
            ...
            .then((response)=>{
                ...
                let data = response.data;
                // 3. update the dim and resize the grid div
                this.dim.metricNum = this.type == 'metrics'? 4:1;
                this.dim.rowNum = data.length;
                let gridWid = this.dim.dataWid+this.dim.metricsWid*this.dim.metricNum+this.dim.modelWid;
                let gridHei = this.dim.titleHei+this.dim.axisHei+(this.dim.rowNum+1)*this.dim.rowHei;
                this.gridSelector = this.containerSelector.append('div').classed('faiMetricsFDiv', true)
    ...
    renderTitleRows(){
        ...
        this.gridSelector.append('div').style('grid-row', '1/span 2').style('grid-column', '1/span 1')
            ...
            .text('Training Data');
        this.gridSelector.append('div').style('grid-row', '1/span 2').style('grid-column', '2/span 1')
            .classed('titleBBor', true).classed('Seperator', true).classed('title', true).classed('metricsBg', false)
            .text('Model');
        // the metric names
        for(let i = 0; i < this.dim.metricNum; i++){
            this.gridSelector.append('div').classed('metricTitle', true).classed('rBor', true).classed('metricsBg', true)
                .text(()=> this.type == 'acc'? 'Accuracy': fairMetricInfo[this.metricNames[i]].fullName);
        }
    ...
    renderAxis(divSelector, type){
        let divWid = parseInt(divSelector.style('width'));
        let divHei = parseInt(divSelector.style('height'));
        let gap = 10;
    ...
    renderMetricVlaues(divSelector, type, value){
        ...
        if(type!='acc'){
            ...
            let yCenter = divHei/2;
            // visualize a bar
            divSvg.append('rect')
                .attr('x', XScale(range[0])).attr('y', yCenter-barLen/2)
                .attr('width', XScale(value)-XScale(range[0])).attr('height', barLen)
                .attr('fill', fontColor)
                .attr('fill-opacity', 0.2);

            // visualize the bars
            divSvg.append('line')
                .attr('x1', XScale(value)).attr('y1', yCenter-barLen/2)
                .attr('x2', XScale(value)).attr('y2', yCenter+barLen/2)
                .attr('stroke', fontColor)
                .attr('stroke-width', '3px');

            // visualize the value
            divSvg.append('text')
                .attr('x', XScale(value)+5).attr('y', yCenter)
                .attr('dy', '0.5em')
    ...
    renderLegend(){
        let baisAreaColor = '#EDEDED';  
        let fontColor = '#213547';
        let emptyNum = this.metricNames.length;
