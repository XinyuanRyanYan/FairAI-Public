...
class TrainAndTestPanel{
     ...
     */
    constructor(templateId, trainName, modelName, finalProj=false, setPosition, VCSet){
        ...
        if(this.modelName!='LR' && !finalProj){
            this.activate();
        }
     ...
     * activate this model
     */
     async activate(trainName='', modelName=''){
        if(trainName){this.trainName = trainName;}
        if(modelName){this.modelName = modelName;}
        this.MLPipelineDiv.style('display', 'grid');    // reveal the div
        
        if(this.finalProj){
            this.trainData = this.VCSet.data.trainData[trainName];
            this.testData = this.VCSet.data.testData;
            // console.log('train and test data are', this.trainData, this.testData);
            this.init();
        ...
        this.MLPipelineDiv.style('display', 'grid');    // reveal the div
        // get the baseline of confusion matrix, and accuray; (used for the mitigation part)
        
        // if chapter1, then get the train (original) and test data from the VCSet
        if(curChapterId==1){
    ...
    init(){
        ...
        if(this.trainName=='Original'){
            visTrainOrTestData(this.trainDiv, this.trainData, `Training Data`, '', this.finalProj?['Married', 'Unmarried']:'');
        }
            ...
            this.VCSet.data.trainData['Reweighing'], this.finalProj?['Married', 'Unmarried']:'');
        }
        else{
            visTrainOrTestData(this.trainDiv, this.trainData, `Training Data after ${this.trainName}`, '', this.finalProj?['Married', 'Unmarried']:'');
        }
        visTrainOrTestData(this.testDiv, this.testData, 'Test Data', '', this.finalProj?['Married', 'Unmarried']:'');
        ...
        console.log('legendHei', legendHei);
        ...
        let trainHei = parseFloat(this.inputDiv.select('.trainDataDiv').style('height'));
        let testHei = parseFloat(this.inputDiv.select('.testDataDiv').style('height'));
        console.log('trainHei', trainHei);
        console.log('testHei', testHei);
        ...
        this.trainModelLine.append("line")
            .attr("marker-end", "url(#trainArrowheadId)")
            .attr("x1", svgWid/2)
            .attr("y1", legendHei+trainHei/2)
            .attr("x2", svgWid/2)
        ...
        this.evalModelLine = arrowG.append("line")
            .attr("marker-end", "url(#testArrowheadId)")
            .attr("x1", 0)
    ...
    async trainModel(){
        ...
        // 2. train
        let link = curChapterId==1? '/trainModelC1':'/trainModel';
    ...
    async testModel(){
        ...
        let link = curChapterId==1? '/testModelC1':'/testModel';
        /**
         * The result of this model:
         * {accuracy: 0.725, attrVs: ['Male', 'Female'] data: [Array(2), Array(2)]}
         */
        await axios.post(link, {'dataName': 'test', 'train': name, 'model': this.modelName})
            .then((response)=>{ 
                // enableChapterBtns();           
                ...
                if(curChapterId==1){this.lastAccuracy = accuracy;}  // change the lastAccuracy only in the chapter1 
                ...
                visOutputPanel(this.outputDiv, response.data, this.colorMapDiv, '', '', accChange, malePredChange, femalePredChange, this.finalProj?['Married', 'Unmarried']:'');
    ...
    trainStyle(){
        ...
        this.modelCNTsvg.select('#testArrowheadId').select('path').style('fill', null);

        this.trainDiv.classed('div-highlight', true);       
        this.trainDiv.select('.plotTitle').style('color', this.highlightColor);
        this.modelCNTsvg.select('#trainArrowheadId').select('path').style('fill', this.highlightColor);
   ...
   /**
    ...
    testStyle(){
        ...
        this.modelCNTsvg.select('#trainArrowheadId').select('path').style('fill', null);
        ...
        this.modelCNTsvg.select('#testArrowheadId').select('path').style('fill', this.highlightColor);
